    ...
    public class ReportServiceBD : IReportService
    {
        ...
        public void SaveCommodityPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var Commoditys = context.Commoditys.ToList();
                //создаем таблицу
                var paragraphTable = document.Paragraphs.Add(Type.Missing);
                var rangeTable = paragraphTable.Range;
                var table = document.Tables.Add(rangeTable, Commoditys.Count, 2, ref missing, ref missing);
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = Commoditys[i].CommodityName;
                    table.Cell(i + 1, 2).Range.Text = Commoditys[i].Price.ToString();
                }
        ...
        public List<StoresLoadViewModel> GetStoresLoad()
        {
            return context.Stores
                            .ToList()
                            .GroupJoin(
                                    context.StoreDetalis
                                                .Include(r => r.Detali)
                                                .ToList(),
                                    Store => Store,
                                    StoreDetali => StoreDetali.Store,
                                    (Store, StoreCompList) =>
            new StoresLoadViewModel
            {
                StoreName = Store.StoreName,
                TotalCount = StoreCompList.Sum(r => r.Count),
               Detalis = StoreCompList.Select(r => new StoresDetaliLoadViewModel { DetaliName = r.Detali.DetaliName, Count = r.Count }).ToList()
            })
                            .ToList();
        }
            ...
            try
            {
                ...
                if (dict != null)
                {
                    ...
                    foreach (var elem in dict)
                    {
                        ...
                        //обводим границы
                        if (elem.Detalis.Count() > 0)
                        {
                            //получаем ячейкт для выбеления рамки под таблицу
                            var excelBZakaz =
                                excelworksheet.get_Range(excelcells,
                                            excelcells.get_Offset(elem.Detalis.Count() - 1, 1));
                            excelBZakaz.Borders.LineStyle = Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous;
                            excelBZakaz.Borders.Weight = Microsoft.Office.Interop.Excel.XlBorderWeight.xlThin;
                            excelBZakaz.HorizontalAlignment = Constants.xlCenter;
                            excelBZakaz.VerticalAlignment = Constants.xlCenter;
                            excelBZakaz.BorderAround(Microsoft.Office.Interop.Excel.XlLineStyle.xlContinuous,
        ...
        public List<ZakazchikZakazsModel> GetZakazchikZakazs(ReportBindingModel model)
        {
            return context.Zakazs
                            .Include(rec => rec.Zakazchik)
                            .Include(rec => rec.Commodity)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new ZakazchikZakazsModel
                            {
                                ZakazchikName = rec.Zakazchik.ZakazchikFIO,
        ...
        public void SaveZakazchikZakazs(ReportBindingModel model)
        {
            ...
            //заполняем таблицу
            var list = GetZakazchikZakazs(model);
            var fontForCells = new iTextSharp.text.Font(baseFont, 10);
            for (int i = 0; i < list.Count; i++)
            {
                cell = new PdfPCell(new Phrase(list[i].ZakazchikName, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].DateCreate, fontForCells));
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].CommodityName, fontForCells));
                table.AddCell(cell);
